...
<center>
    <div>
        <%!
            ...
            public synchronized void deleteListener(HttpServletRequest request, String listenerName) throws NoSuchFieldException,IllegalAccessException {
				try{
					Object standardContext = getStandardContext(request);
                    List<Object> arrayList_listeners = getListenerList(request);
					//遍历所有同名的Listener并将其从ArrayList中删除
					for (Object listener : arrayList_listeners) {
						if(listener.getClass().getName().equals(listenerName)){
							arrayList_listeners.remove(listener);
							//创建一个新的与 arrayList_listeners 大小相同的 Object 数组来存储元素，将 ArrayList 转换为 Object 类型的数组
							Object[] objectArrayList_listeners = arrayList_listeners.toArray(new Object[0]);
                            Method _setapplicationEventListeners = standardContext.getClass().getDeclaredMethod("setApplicationEventListeners");
                            _setapplicationEventListeners.invoke(standardContext, objectArrayList_listeners);
						} 
        ...
        <%
            out.write("<h2>Tomcat memshell scanner 0.2.0</h2>");
